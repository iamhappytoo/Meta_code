...
def count_of_nodes(root, queries, s):
  ...
  for query,val in queries:
    key_node=findNode(root,query)
    output.append(count_nodes(key_node,val,s))
  return output

def findNode(root,query):
  if root is not None:
    if root.val==query:
      return root
    else:
      for child in root.children:
        if findNode(child,query):
          return findNode(child,query)

def count_nodes(node,val,s):
  if node is None:
    return 0
  else:
    count=0
